...
Le domaine métier (du point de vue des utilisateurs de l'application) est définis par un enseble de classes `Entity` placées dans un package à part (très souvent nommé *model* footnote:[Na pas confondre avec la classe `Model` de `MVC` (_Model View Controller_), qui est une classe qui a pour fonction de liaison entre le contrôleur et la vue.]). Ce sont les données qui font l'objet de persistance dans les bases de données.
...
class Article(
    ...
    var headline: String,
    var content: String,
    @ManyToOne var creditedTo: Person,  <5>
...
Typiquement, le formulaire HTML est l'outil qui permet à un utilisateur, in fine, de créer ou modifier une entité du modèle métier.
...
<2> Le paramètre de type `PersonDto` sera passé à cette fonction au moment de son appel. Cet objet sera soit créé (la première fois) soit extrait de l'objet `Model` du contexte d'appel (celui qui fait le lien entre le contrôleur et la vue, et qui est *implicite* ici)
<3> Le chemin vers la vue (de racine implicite `resources/template/`)


=== Validation du formulaire

Ce travail doit être réalisé côté serveur footnote:[même si certaines données peuvent l'être côté client, parfois naïvement]
...
<2> Le paramètre de type `PersonDto` est préfixé par `@Valid`, ce qui a pour effet de valider cet objet avant de le passer à ce contrôleur.
<3> L'objet de type `BindingResult` détient les erreurs, si elles ont été détectées lors de la phase de validation.
...
+
Pour comprendre les problèmes résolus par `PRG` une recherche sur wikipedia s'impose...
